...
Sobre esta base, la Dra. R. Taberner ha definido un vocabulario dermatoscópico propio en castellano, más amplio que las escalas clásicas: una matriz que asocia **37 estructuras dermatoscópicas** (retículo pigmentado, velo blanco-azulado, telangiectasias arboriformes, lagunas rojas, estructuras en hoja de arce, lágrimas amarillas o el signo del ala delta, entre otras) con **15 diagnósticos dermatoscópicos**, indicando qué buscar en cada uno. Esta matriz constituye la **verdad de terreno conceptual definitiva** del módulo SAE y se documenta íntegra en [`ground-truth-dermatoscopia-rosa.md`](ground-truth-dermatoscopia-rosa.md) (artefacto legible por máquina en [`dermatoscopia_estructuras_rosa.json`](dermatoscopia_estructuras_rosa.json)).

El modelo tiene tres capas de anotación con semántica distinta: **asimetría** (sí/no), **color** (once colores detectados automáticamente por el algoritmo, no por anotación experta) y **estructuras** (37). La aportación clínica clave reside en las estructuras pigmentadas discriminantes ---retículo pigmentado, glóbulos marrones, proyecciones lineales/pseudópodos y mancha de pigmento---, que **no** se anotan como presente/ausente sino como **típico/atípico**: la mera presencia de una estructura no es patológica; lo que discrimina (p. ej. nevus frente a melanoma sobre la misma estructura) es si es *típica* (benigna) o *atípica* (signo de alarma). Esta granularidad, que una sonda binaria presencia/ausencia no capta, es la dirección de validación conceptual prioritaria del SAE. Este vocabulario aún no se ha empleado para entrenar (trabajo en curso); su finalidad es que, en el futuro, las explicaciones del modelo se expresen en los mismos términos que un dermatólogo emplearía ante el paciente.
